...
- (void)showLockScreenTotaltime:(float)totalTime andCurrentTime:(float)currentTime andLyricsPoster:(BOOL)isShow{
    ...
    if (isShow) {
        ...
        if (!_lockScreenTableView) {
            _lockScreenTableView = [[UITableView alloc] initWithFrame:CGRectMake(20, 800 - 44 * 7 + 20, 480, 44 * 3) style:UITableViewStyleGrouped];
...
-(void)addThreebtn{
    NSArray * arr=@[@"before",@"stop",@"next"];
    for (int i=0; i<3; i++) {
        UIButton * btn=[[UIButton alloc]initWithFrame:CGRectMake(100*i, self.view.frame.size.height-100, (self.view.frame.size.width-100)/3, 40)];
